...
namespace SV20T1020085.Web.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        /// <summary>
        /// Chuyển trạng thái sang trạng thái bị hủy
        /// </summary>
        ...
        /// <summary>
        /// Chuyển trạng thái đơn hàng sang trạng thái bị từ chối
        /// </summary>
        /// <param name="id">Mã đơn hàng</param>
        /// <returns></returns>
        public IActionResult Reject(int id = 0)
        {
            ...
            if (!result)
            {
                TempData["Message"] = "Không thể thực hiện thao tác từ chôis đối với đơn hàng này";
            }
            return RedirectToAction("Details", new { id });
        }
        /// <summary>
        ...
        /// <summary>
        /// Giao điện để sửa đổi thông tin mặt hàng được bán trong đơn hàng
        /// </summary>
        /// <param name="id">Mã đơn hàng</param>
        /// <param name="productId">Mã đơn hàng</param>
        /// <returns></returns>
        [HttpGet]
        ...
        /// <summary>
        /// Cập nhật gias bán và ssoos lượng bán của 1 mặt hàng được bans trong đơn hàng
        ...
        /// <param name="ssalePrice">Giá bans</param>
        /// <returns></returns>
        [HttpPost]
        public IActionResult UpdateDetail(int orderID, int productID, int quantity, decimal salePrice)
        {
                ...
                bool result = false;
                result = OrderDataService.SaveOrderDetail(orderID, productID, quantity, salePrice);
                
                if (!result)
                    return Json("không được phép thay đổi thông tin đơn hàng");
                return Json("");
        }
        /// <summary>
        ...
        public IActionResult Shipping(int id=0, int shipperID=0)
        {
            if (shipperID == 0)
            {
                return Json("Vui lòng chon người giao hàng");
            }
        ...
        // Số dòng trên 1 trang khi hiển thị danh sách mặt hàng cần tìm khi mới lập đơn hàng
        ...
        private const string SHOPPING_CART = "shpping_cart";
        /// <summary>
        ...
        /// <summary>
        /// Lấy giỏ hàng hiên đang lưu trong session
        /// </summary>
        /// <returns></returns>
        ...
        public IActionResult AddToCart(OrderDetail data) 
        {
            ...
            var existsProducts = shoppingcart.FirstOrDefault(m => m.ProductID == data.ProductID);
            if(existsProducts == null)
                shoppingcart.Add(data);//Nếu mặt hàng chưa có trong giỏ thì bổ sung thêm vào giỏ hàng
            else // Nếu mặt hàng đã có trong giỏ thì tăng số lượng và thay đổi giá bán
            {
                existsProducts.Quantity += data.Quantity;
                existsProducts.SalePrice = data.SalePrice;
            }
            ApplicationContext.SetSessionData(SHOPPING_CART, shoppingcart);
        ...
        public IActionResult Init(int customerID =0, string deliveryProvince ="", string deliveryAddress="") {
            try {
                ...
                {
                    return Json("Vui lòng nhập dầy đủ thông tin");
                }

                int employeID = Convert.ToInt32(User.GetUserData()?.UserId);
                int orderID = OrderDataService.InitOrder(employeID, customerID, deliveryProvince, deliveryAddress, shoppingcart);
                ...
                return Json("Lỗi máy chủ nội bộ.");
            }
